...
public class VF2 {
	...
	// 带关系的匹配
	public Vector<Node[]> findIso(Vector<Node> in_n, Vector<Node> in_m, Vector<Node> out_n, Vector<Node> out_m,
			Vector<Node> core_n, Vector<Node> core_m, Node nn, Node mm, Graph NG, Graph MG){
		Vector<Node[]> canIso = new Vector<Node[]>();
		if(in_n.size()>0 && in_m.size()>0){
			Node n2 = in_m.firstElement();
			String n2label = n2.GetAttribute_str();
			if(MG.getEdge(n2, mm) != null){
				for(Node n1 : in_n){
					if(NG.getEdge(n1, nn) != null){
						String n1EdgeType = NG.getEdge(n1, nn).getType();
						String n2EdgeType = MG.getEdge(n2, mm).getType();
						String n1label = n1.GetAttribute_str();
						if(n2label.equals(n1label) || n2label.equals("*")){
							if(n1EdgeType.equals(n2EdgeType)) {
								Node[] pair = {n1, n2};
								canIso.add(pair);	
							}
						}	
				...
				//和已匹配上的节点判断关系
				for(int i=0; i<core_n.size(); i++) {
					if(MG.getEdge(n2, core_m.get(i))!=null) {
						for(Node n1 : in_n){
							if(NG.getEdge(n1, core_n.get(i)) != null){
								String n1EdgeType = NG.getEdge(n1, core_n.get(i)).getType();
								String n2EdgeType = MG.getEdge(n2, core_m.get(i)).getType();
								String n1label = n1.GetAttribute_str();
								if(n2label.equals(n1label) || n2label.equals("*")){
									if(n1EdgeType.equals(n2EdgeType)) {
										Node[] pair = {n1, n2};
			...
			String n2label = n2.GetAttribute_str();
			if(MG.getEdge(mm, n2) != null){
				for(Node n1 : out_n){
					if(NG.getEdge(nn, n1) != null){
						String n1EdgeType = NG.getEdge(nn, n1).getType();
						String n2EdgeType = MG.getEdge(mm, n2).getType();
						String n1label = n1.GetAttribute_str();
						if(n2label.equals(n1label) || n2label.equals("*")){
							if(n1EdgeType.equals(n2EdgeType)) {
								Node[] pair = {n1, n2};
				...
				//和已匹配上的节点判断关系
				for(int i=0; i<core_n.size(); i++) {
					if(MG.getEdge(core_m.get(i), n2)!=null) {
						for(Node n1 : out_n){	
							if(NG.getEdge(core_n.get(i), n1) != null){
								String n1EdgeType = NG.getEdge(core_n.get(i), n1).getType();
								String n2EdgeType = MG.getEdge(core_m.get(i), n2).getType();
								String n1label = n1.GetAttribute_str();
								if(n2label.equals(n1label) || n2label.equals("*")){
									if(n1EdgeType.equals(n2EdgeType)) {
										Node[] pair = {n1, n2};
		...
		else if(in_n.size()==0 && in_m.size()==0 && out_n.size()==0 && out_m.size()==0 && nn==null && mm==null){	// used for initialisation
			//Node u = this.Q.FindANode();
			Node u = this.Q.FindANodeNotStart(); //查询图中随机找在某个点,且保证该节点的的属性不能为*
			for(Node v : this.G.GetNodeSet().values()){
				if(u != null && v != null){
					if(u.GetAttribute_str().equals(v.GetAttribute_str())){ //属性和标签都一致
						Node[] pair = {v, u};
						canIso.add(pair);
					}
				}
	...
	public Vector<Node[]> findIso(Vector<Node> in_n, Vector<Node> in_m, Vector<Node> out_n, Vector<Node> out_m,
			Vector<Node> core_n, Vector<Node> core_m){
		Vector<Node[]> canIso = new Vector<Node[]>();
		if(in_n.size()>0 && in_m.size()>0){
			Node n2 = in_m.firstElement();
			String n2label = n2.GetAttribute_str();
		...
		else if(in_n.size()==0 && in_m.size()==0 && out_n.size()==0 && out_m.size()==0){	// used for initialisation
			//Node u = this.Q.FindANode();
			Node u = this.Q.FindANodeNotStart(); //查询图中随机找在某个点,且保证该节点的的属性不能为*
			for(Node v : this.G.GetNodeSet().values()){
				if(u != null && v != null){
	...
	/**
	 * @param n : in NG, shall be added in the partial matching
	 * @param m : in MG, shall be added in the partial matcing
	 * @param core_n : partial matching, including nodes in NG
	 * @param core_m : partial matching, including nodes in MG
	 * @param NG : data graph
	 * @param MG : pattern graph
	 * @return
	 */
	public boolean verifyR_pred(Node n, Node m, Vector<Node> core_n, Vector<Node> core_m, Graph NG, Graph MG){
		boolean R_pred = true;
	...
	public boolean verifyR_succ(Node n, Node m, Vector<Node> core_n, Vector<Node> core_m, Graph NG, Graph MG){
		boolean R_succ = true;
		...
		if((childrenN.isEmpty() && !childrenM.isEmpty()) || (!childrenN.isEmpty() && childrenM.isEmpty())){
			return false;
		}
		else{
	...
	public boolean verifyR_in(Node n, Node m, Vector<Node> in_n, Vector<Node> in_m, Graph NG, Graph MG)	{
	...
	public boolean verifyR_out(Node n, Node m, Vector<Node> out_n, Vector<Node> out_m, Graph NG, Graph MG)	{
		HashSet<Node> predN = NG.GetParents(n);
		HashSet<Node> succN = NG.GetChildren(n);
		HashSet<Node> predM = MG.GetParents(m);
		HashSet<Node> succM = MG.GetChildren(m);
	...
	public boolean verifyR_new(Node n, Node m, Vector<Node> core_n, Vector<Node> core_m, Vector<Node> in_n, Vector<Node> in_m, Vector<Node> out_n, Vector<Node> out_m, Graph NG, Graph MG){
		HashSet<Node> predN = NG.GetParents(n);
		HashSet<Node> succN = NG.GetChildren(n);
		HashSet<Node> predM = MG.GetParents(m);
		HashSet<Node> succM = MG.GetChildren(m);
...
//		LinkedList<Node> MVset = MG.GetNodeSet();
	...
	public void Match(Vector<Node> core_1, Vector<Node> core_2,
			Vector<Node> in_1, Vector<Node> in_2, Vector<Node> out_1, Vector<Node> out_2, Node n1,
			Node n2, Graph NG, Graph MG){
		HashSet<String> ans = new HashSet();
		if(core_2.containsAll(this.Q.GetNodeSet().values())){
			for(int i=0; i<core_1.size(); i++){
//				System.out.println("Matches: " + core_1.get(i).GetID()+  ", " + core_1.get(i).GetAttribute_str() + " : " + core_2.get(i).GetID() + ", " +core_2.get(i).GetAttribute_str());
				if(core_2.get(i).GetAttribute_str().equals("*")){
			    	System.out.println("answer： "+core_1.get(i).GetAttribute_str());
			    	ans.add(core_1.get(i).GetAttribute_str().trim());
			    }
			}
			this.num ++;
			...
			Vector<Node[]> canIso = this.findIso(in_1, in_2, out_1, out_2, core_1, core_2, n1, n2, NG, MG);
			for(Node[] pair : canIso){
				if(this.testFeasibility(pair, core_1, core_2, in_1, in_2, out_1, out_2)){
					...
					in_2_dup.removeAll(core_2_dup);		/**	in_2 不能包含core_2中的点 */
					
					// syn out_2
					HashSet<Node> childrenM = this.Q.GetChildren(m);
					out_2_dup.addAll(childrenM);
					out_2_dup.removeAll(core_2_dup);	/**	out_2 不能包含core_2中的点 */
					this.Match(core_1_dup, core_2_dup, in_1_dup, in_2_dup, out_1_dup, out_2_dup, n, m, this.G, this.Q);
				}
			}
		}
	}
	...
	public void Match_II(Vector<Node> core_1, Vector<Node> core_2,
			Vector<Node> in_1, Vector<Node> in_2, Vector<Node> out_1, Vector<Node> out_2){
		if(core_2.containsAll(this.Q.GetNodeSet().values())){
			this.flag = true;
		}
		else{
